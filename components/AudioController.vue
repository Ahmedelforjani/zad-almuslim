...
const currentTimeRef = ref<number[] | undefined>([audio.currentTime]);
...
<template>
  <div class="space-y-4 max-lg:ms-auto" v-if="track">
    <div class="flex items-center justify-center gap-4">
      <Button size="icon" variant="ghost" class="rounded-full flex" @click="audio.nextTrack()">
      ...
      <Button size="icon" variant="ghost" class="rounded-full flex" @click="audio.prevTrack()">
        <Icon name="lucide:skip-back" mode="svg" class="[&>path]:fill-current h-5 w-5" />
      </Button>
    </div>
    <div :class="cn('text-sm text-muted-foreground flex gap-4', props.seekBarClass)" v-if="
      track.type !== 'broadcast' && audio.duration
    ">
